...
class TicTacToeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Tic Tac Toe vs MCTS AI")
        self.game = gameState()
        self.ai = MCTS(loops=1000)
        ...
        for i in range(3):
            for j in range(3):
                btn = tk.Button(self.root, text="", font=("Arial",30), width=5, height=2,
                                command=partial(self.human_move,i,j))
                btn.grid(row=i,column=j)
                self.buttons[i][j] = btn
        self.status = tk.Label(self.root,text="", font=("Arial",16))
    ...
    def human_move(self,i,j):
        if self.game.player!=1 or self.game.board[i][j]!=0: return
        self.game.playMove(i,j)
        self.update_buttons()
        self.check_game_over()
        self.root.after(100, self.ai_move)

    def ai_move(self):
        if self.game.player!=2: return
        move=self.ai.search(self.game)
        self.game.playMove(move[0],move[1])
        self.update_buttons()
        self.check_game_over()
    ...
    def update_buttons(self):
        for i in range(3):
            for j in range(3):
                ...
                if val==1: self.buttons[i][j].config(text="O")
                elif val==2: self.buttons[i][j].config(text="X")

    def check_game_over(self):
        res=self.game.check_terminated()
        if res is not None:
            if res==0: msg="Draw!"
            elif res==1: msg="You win!"
            else: msg="AI wins!"
